...
- [x] **3D 化**: 描画を Canvas 2D → Three.js (WebGL) に移植
- [x] **Phase 1.5**: 街景観の高密度化 (建物 tier 1-15、多色パレット、街灯、横断歩道、桜、広場、川エッジ)
- [x] **Phase 1.75**: 動きと拡張 (走行車・駐車車両・噴水/川/雲アニメ・窓またたき・松・玄関/生垣・地面色ムラ・32×32) ← **ここ完了**
...
- **タイル定義**: `grid[y][x] = { type, population, ... }`。type は
  `'empty' | 'road' | 'residential' | 'plaza'`。新しいプロパティを追加する時は
  `state.js` の `makeEmptyGrid` も更新
- **CONFIG.VISUAL は装飾専用**: パレット・tier 閾値・桜の割合などはここに集約。
  `simulation.js` から絶対に読まないこと (シムと装飾の独立を維持)
