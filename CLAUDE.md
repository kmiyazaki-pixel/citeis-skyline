...
1. **グラフィックは Canvas 描画 or CSS図形のみ** — 画像ファイルは使わない
...
- [x] **Phase 1**: グリッド + 道路/住宅/取壊 + 基本経済シミュ ← **ここ完了**
- [ ] **Phase 2**: 商業ゾーン + 雇用システム
...
- **Canvas は1つだけ**: ワールド描画は単一の `<canvas>`。UI は DOM。
  オーバーレイが必要になっても安易に複数キャンバスを増やさず、まず1つで頑張る
...
- **座標系**: `x` は横、`y` は縦。配列は `grid[y][x]` の順 (描画ループと一致)
...
6. **`renderer.js`** に commercial の描画を追加 (青系、occupancy で濃淡)
...
- Canvas の描画コードは `renderer.js` に集約。他のファイルから直接 ctx を触らない
- レスポンスは日本語で、簡潔に
