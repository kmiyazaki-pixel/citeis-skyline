...
└── src/
    ...
    ├── save.js         localStorage セーブ/ロード (位置・資材・能力・構造物)
    ├── structures.js   拠点の建物キット (土台。プリミティブのみ)
    ├── build.js        拠点づくりモード (ゴースト/設置/資材消費)
    └── hud.js          HUD表示、タイトル画面、一時停止/設定、拠点UI
```
...
- [~] **拠点づくり (進行中)**: 「資材を集めて拠点を発展」要素のMVP
  - [x] 土台の設置システム (build.js / structures.js)
    - クリスタルを資材化: `state.crystals`=手持ち(消費)、`state.crystalsTotal`=累計(能力解放)
    - ビルドモード (🔨/Bキー) → 前方にゴースト → 設置 (E/クリック/設置ボタン)
    - 傾斜・水際・資材不足を判定 (ゴーストが緑/赤)
    - `state.structures` に永続化、つづきからで復元
  - [ ] 次: 資材採取の強化 (木/岩から採れる/採取量UPの道具)、建物の種類追加
        (小屋/かまど/橋/灯り)、村人 (ウサギ徘徊を流用)、拠点ティア (camp→village)
- 個別の積み残し: バイオーム地形分け、祠クエスト、岸の泡、構造物の解体
