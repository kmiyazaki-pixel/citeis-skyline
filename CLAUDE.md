...
└── src/
    ...
    ├── sky.js          昼夜サイクル (空ドーム/太陽/月/星/霧/ライト/露出)
    ├── creatures.js    野ウサギ (徘徊 + 逃走)
    ├── audio.js        WebAudio 合成 (効果音・足音・環境音・BGM)
    ├── vfx.js          粒子演出 (ホタル)
    ├── settings.js     設定の永続化と適用 (音量/感度/影)
    ├── save.js         localStorage セーブ/ロード (位置・資材・道具・能力・構造物)
    ├── gather.js       採取 (木→木材/岩→石)、道具強化、能力解放
    ├── structures.js   拠点の建物キット (土台/小屋/かまど/橋/灯り。プリミティブのみ)
    ├── build.js        拠点づくりモード (パレット/ゴースト/設置/資材消費)
    └── hud.js          HUD表示、タイトル画面、一時停止/設定、採取/拠点UI
```
...
- [x] **Tier 2**: 本物のゲーム化 ← **ここ完了**
  ...
  - 留保: 祠クエストの連鎖、岸の泡 (次パス)
- [~] **Tier 3**: 出荷準備 (進行中)
  - [x] 木/岩の衝突判定 (world.resolveCollision、円-円押し出し)
...
- [x] **採取＆拠点づくり** ← **ここ完了**
  - クリスタルを廃止 (収集が手間だったため)。資材は採取式に
  - 採取 (gather.js): 木→木材🪵 / 岩→石🪨。E/クリック/採取ボタン、最寄りノード、
    採取量は道具レベル ×(1+lv)。累計採取数で能力解放 (12/36/80)
  - 道具強化: 資材を払って採取量を増やす (🔧 Lv0→3)
  - 建築パレット (build.js): 土台/小屋/かまど/橋/灯り を Q/◀▶ で切替、木材石を消費
  - 設置: ビルドモード (🔨/B) → ゴースト (緑=可/赤=不可、傾斜・水際・資材判定) →
    設置 (E/クリック/設置ボタン)。`state.structures` に永続化、つづきからで復元
  - state: `wood`/`stone`(手持ち)・`gatheredTotal`(累計)・`toolLevel`・`structures`
- [ ] 次: 採取ノードの枯渇/再生、村人 (ウサギ徘徊を流用して小屋に住む)、
      拠点ティア (camp→village)、建物の機能 (かまどで道具クラフト等)、構造物の解体
